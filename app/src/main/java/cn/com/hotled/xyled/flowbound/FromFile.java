...
public class FromFile extends BaseFlowBound {
    ...
    public List<byte[]> genFlowBound() {
        ...
        Bitmap bitmap = BitmapFactory.decodeFile(flowFile.getAbsolutePath());
...
//                    Log.i("move","no color,x = "+x1+", y= "+y1);
        ...
        for (int i = 0,index =0; i < frameCount; i++,index++) {
            ...
            for (int k = 0; k < screenWidth - index; k++) {
                int colorIndex = (k + index)*bitmap.getHeight();//this is right
                while (colorIndex >= flow.length) {
                    colorIndex -= flow.length;
                }
                for (int q=0;q<bitmap.getHeight();q++){
            ...
            for (int k = 0; k < index; k++) {
                int colorIndex = (index - k)*bitmap.getHeight();
                while (colorIndex >= flow.length) {
                    colorIndex -= flow.length;
                }
                for (int q=0;q<bitmap.getHeight();q++){
            ...
            for (int j = 0; j < screenHeight-colIndex; j++) {
                int colorIndex = j * bitmap.getHeight();
                while (colorIndex >= flow.length) {
                    colorIndex -= flow.length;
                }
                for (int q = 0; q < bitmap.getHeight(); q++) {
            ...
            for (int j = 0; j <colIndex; j++) {
                int colorIndex= (screenHeight-colIndex+j)*bitmap.getHeight();
                while (colorIndex>=flow.length){
                    colorIndex-=flow.length;
                }
                for (int q = 0; q < bitmap.getHeight(); q++) {
            ...
            for (int k=screenWidth;k>=2;k--){
                int colorIndex=k*bitmap.getHeight();
                while (colorIndex>=flow.length){
                    colorIndex-=flow.length;
                }
                if (k>index){
                    for (int q=bitmap.getHeight();q>0;q--)
            ...
            //left part1
            for (int j = 0; j < screenHeight-colIndex; j++) {
                int colorIndex=(colIndex+j)*bitmap.getHeight();
                while (colorIndex>=flow.length){
                    colorIndex-=flow.length;
                }
                for (int q = 0; q < bitmap.getHeight(); q++) {
            ...
            for (int j = 0; j <colIndex; j++) {
                ...
                for (int q = 1; q <= bitmap.getHeight(); q++) {
