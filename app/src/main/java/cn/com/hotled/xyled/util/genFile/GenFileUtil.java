...
public class GenFileUtil {
    ...
    public GenFileUtil(Activity context, List<Program> programs, int screenWidth, int screenHeight, Handler handler) {
        mContext = context;
    ...
    private void initTimeAxis() {
        ...
        for (int i = 0; i < mProgramList.size(); i++) {//图片时间轴
    ...
    private void setTextProgramTimeAxis(int textProgramIndex,int programIndex) {
        ...
        for (int i = 0; i < frameOfThisProgram; i++) {
            ...
            if (i==frameOfThisProgram-1){
                //如果是节目的最后一帧，则压缩的mTempColbyteCount应该加上还没有的录入的CompressUtil
    ...
    public void startGenFile() {
        new Thread(new GenFileUtil.GenFileThread()).start();
    }
