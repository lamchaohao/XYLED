...
public class GenFlow extends BaseFlowBound{
    ...
    public GenFlow(Context context, int screenWidth, int screenHeight) {
    ...
    public void setProgramLength(int[] lengths){
        programLengthArrays=lengths;
    }
    ...
    private void setToBackground() {
        ...
        for (int i = 0; i < useFlows.length; i++) {
            if (useFlows[i]) {
                ...
                setFlowToBack(flow,mBitmapHeightList.get(flowFileIndex),programLengthArrays[i],flowEffect[flowFileIndex]);
                ...
                for (int j = 0; j < programLengthArrays[i]; j++) {
